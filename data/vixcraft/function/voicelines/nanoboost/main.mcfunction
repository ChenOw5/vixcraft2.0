...
execute as @a[tag=nano,tag=bastet] at @s run tellraw @a {"text":"<Bastet> I'm too old for this","color": "yellow"}
execute as @a[tag=nano,tag=sorcerer] at @s run tellraw @a {"text":"<Sorcerer> Priotise the elderly next time will you?","color": "dark_purple"}
execute as @a[tag=nano,tag=cryomancer] at @s run tellraw @a {"text":"<Cryomancer> Into the eternal cold","color": "aqua"}